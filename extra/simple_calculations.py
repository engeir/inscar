...
def ions(path):
    ...
    # l_D = np.sqrt(const.epsilon_0 * const.k * keys['T_E'] / (keys['NE'] * const.e**2))
    l_D = np.sqrt(const.epsilon_0 * const.k *
                  keys['T_I'] / (keys['NE'] * const.e**2))
    # a = 1 / l_De**2 + 1 / l_Di**2
    # l_D = np.sqrt(1 / a)
    w = k0 * v_th / (1 + k0**2 * l_D**2)  # / (2 * np.pi)
    print(
        f'ion thermal velocity = {v_th:1.3e} = {v_th/v_th_e:1.3e}v_th_e\nfrequency = {w:1.3e}')
...
def electrons(path):
    ...
    # l_D = np.sqrt(const.epsilon_0 * const.k * keys['T_E'] / (keys['NE'] * const.e**2))
    w_pe = np.sqrt(keys['NE'] * const.e**2 / (const.m_e * const.epsilon_0))
